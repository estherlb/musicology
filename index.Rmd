...
There are three playlists so far: Theme songs, soundtracks and my favorite songs. I'm going to look at some features. At first danceability and valence, after that tempo and energy.  
...
ggplot(songs, aes(energy, tempo, colour = category, size = track.popularity)) + 
  geom_point()+
  theme_solarized()+
  labs(title = "Tempo vs Energy", caption = "Tempo seems equally distributed for all the three playlists, unlike the other features.")
```
...
ggplot(by_year, aes(x=year, y=energy))+
  geom_smooth()+
  labs(title="Energy over the years", subtitle = "Theme songs", caption = "Energy seems to descend over the years.")+
  theme_wsj()
...
ggplot(by_year, aes(x=year, y=tempo))+
  geom_smooth()+
  labs(title="Tempo over the years", subtitle = "Theme songs", caption = "Tempo has a steady mean.")+
  theme_wsj()
```
